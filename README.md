...
| `copilot/<model>`     | `gh`         | Copilot CLI pinned to a specific model  |
...
**Install:** just the [GitHub CLI](https://cli.github.com/) (`gh`) itself — `gh copilot` is a built-in `gh` command (v2.86+) that auto-downloads the Copilot CLI on first run if it isn't already on your `PATH`. To install the standalone `copilot` binary yourself instead (e.g. to pin a version), run `npm install -g @github/copilot`.

**Authenticate:** run `gh auth login` (if you haven't already) — Copilot CLI reuses your existing `gh` GitHub authentication. Requires an active Copilot subscription (Individual, Business, or Enterprise).
...
| `--agent <agent>`       | `claude \| claude/<model> \| codex \| codex/<model> \| spark \| sparc \| opencode \| opencode/<model> \| antigravity \| antigravity/<model> \| copilot \| copilot/<model>` |
...
| `REPOLENS_AGENT_TIMEOUT_COPILOT`     | unset    | GitHub Copilot CLI per-invocation timeout override. Wins over `REPOLENS_AGENT_TIMEOUT` and mode-specific timeout variables when `--agent copilot` or `--agent copilot/<model>` is selected.                                                                                                                                                                                                                                                                                                                    |
...
| `Missing required command: claude` (or `codex` / `opencode` / `agy` for `--agent antigravity` / `gh` for `--agent copilot`)                                       | Agent CLI not installed                                                                                      | See [Supported Agent CLIs](#supported-agent-clis) for install + auth                                                                                                                |
...
| `Invalid agent: …`                                                                                                                   | Typo in `--agent` value                                                                                      | Must be one of `claude`, `codex`, `spark`, `sparc`, `opencode`, `antigravity`, `copilot`, optionally with a `/<model>` suffix on `claude`, `codex`, `opencode`, `antigravity`, or `copilot` (`spark`/`sparc` take no suffix — use `codex/<model>`) |
