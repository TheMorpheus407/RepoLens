...
- **Summary** — `logs/<run-id>/summary.json`, including per-lens status, iterations, issue counts, `rate_limit_sleep_seconds`, and run-level `stopped_reason` when an agent abort guard stops the run
